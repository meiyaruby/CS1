...
-- Return true if p is true for some v in s, otherwise else false (existential quantification)
...
set_powerset: (eq a) => (s: set a) -> set (set a)
...
set_cardinality (mkSet (h::t)) = S (set_cardinality (mkSet t))
...
-- Return true if p is true for every v in s, otherwise false
-- set_forall: (p: a -> bool) -> (s: set a) -> bool
--given a list of boolean values, return true if all are true, otherwise false
...
--pairelt takes an element h and list b and returns a list of all pairs, (h, e), where e is an element of list b
...
appendlistSet: (eq a) => a -> list (set a) -> list (set a)
appendlistSet v nil = (mkSet (v::nil))::nil
appendlistSet v (h::t) = (set_insert v h)::(appendlistSet v t)

powerlist: (eq a) => list a -> list (set a)
powerlist nil = nil
powerlist (h::t) = (appendlistSet h (powerlist t)) ++ (powerlist t)

set_powerset (mkSet l) = mkSet (powerlist l)
-------------------------------------
